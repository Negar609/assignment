...
function populateDropdownJQ(provinces) {
  $.each(provinces, (provinceAbbr) => {
    $('#provinces')
      .append(`<option value="${totalTax(provinces[provinceAbbr].taxes)}">
        ${provinces[provinceAbbr].name}
      </option>`);
  });
}
...
function pizzaSalesJQ() {
  bindTaxDomJQ();
  populateDropdownJQ(salesTaxData.provinces);
}
...
function pizzaSalesAjax() {
  $.ajax({
    ...
    success: (result) => {
      bindTaxDomJQ();
      populateDropdownJQ(result.provinces);
    },
  });
}
